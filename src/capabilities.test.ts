...
describe("the config-history capability", () => {
  ...
  it("restores a snapshot into its own home and says how many files changed", async () => {
    const ledger = fakeLedger();
    const result = await configLedgerHistory("/plugin-home", { open: () => ledger as never }).restore("bbbb222");
...
describe("the settings action runner", () => {
  it("runs a declared action against this plugin's own home", () => {
    const open = vi.fn(() => fakeLedger() as never);
  ...
  it("refuses an action it does not declare", () => {
...
describe("ensureDataRepo", () => {
  it("creates the data repo and snapshots it under the reason it was given", () => {
    const ledger = fakeLedger();
  ...
  it("never throws, because a lifecycle hook that throws quarantines the plugin", () => {
    const open = () => { throw new Error("not a repo"); };
